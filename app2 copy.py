...
class Query(ObjectType):
    ...
    products_by_owner_email = graphene.List(ProductType, ownerEmail=graphene.String(required=True))
    ...
    def resolve_products_by_owner_email(self, info, ownerEmail):
        try:
            # Fetch products with the given ownerEmail
            products = products_collection.find({"owner_email": ownerEmail})
            return [
                {
                    "id": str(product["_id"]),
                    "name": product["name"],
            ...
            print(f"Error fetching products by owner email: {e}")
            return []
